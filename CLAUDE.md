...
次に、`pic-kn/ai-prompt-builder` のGitHub Issuesで `作業メモ` ラベルのついたIssueを確認すること。
未完了の作業がある場合はそこから引き継ぐ。

## セッション終了・中断時のルール

コミット前に作業が中断される可能性がある場合、または作業が完了した場合は、
`pic-kn/ai-prompt-builder` のGitHub Issueに作業状況を記録すること。

- Issueが存在しない場合：タイトル「作業メモ」、ラベル `作業メモ` で新規作成
- Issueが存在する場合：最新の状況でコメントを追加または本文を更新
- 記録する内容：何を作っていたか、どこまで完了したか、次にやること
- コミット・pushが完了したら「完了」とコメントしてIssueをクローズ
